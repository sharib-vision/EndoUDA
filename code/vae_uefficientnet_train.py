...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 25 08:12:23 2020
...
path_test = './Dataset/Dataset3/target/'
...
conv4 = backbone.layers[342].output #(none,8,8,960)
conv4 = LeakyReLU(alpha=0.1)(conv4)
pool4 = MaxPooling2D((2, 2))(conv4)
pool4 = Dropout(dropout_rate)(pool4)

 # Middle
convm = Conv2D(start_neurons * 32, (3, 3), activation=None, padding="same")(pool4) #(none,4,4,512)

conveff = Conv2D(start_neurons * 8, (3, 3), activation=None, padding="same")(convm)
conveff = LeakyReLU(alpha=0.1)(conveff)

convm = residual_block(convm,start_neurons * 32) #(none,4,4,512)
convm = residual_block(convm,start_neurons * 32) #(none,4,4,512)
convm = LeakyReLU(alpha=0.1)(convm) #(none,4,4,512)

deconv4 = Conv2DTranspose(start_neurons * 16, (3, 3), strides=(2, 2), padding="same")(convm) #(none,none,none,256)
uconv4 = concatenate([deconv4, conv4]) #(8,8,1216)
uconv4 = Dropout(dropout_rate)(uconv4) #(8,8,1216)

uconv4 = Conv2D(start_neurons * 16, (3, 3), activation=None, padding="same")(uconv4) #(8,8,256)
uconv4 = residual_block(uconv4,start_neurons * 16) #(8,8,256)
uconv4 = residual_block(uconv4,start_neurons * 16) #(8,8,256)
uconv4 = LeakyReLU(alpha=0.1)(uconv4)

deconv3 = Conv2DTranspose(start_neurons * 8, (3, 3), strides=(2, 2), padding="same")(uconv4) #(none,none,none,128)
conv3 = backbone.layers[154].output #(none,16,16,336)
uconv3 = concatenate([deconv3, conv3])    #(none,16,16,464)
uconv3 = Dropout(dropout_rate)(uconv3)

uconv3 = Conv2D(start_neurons * 8, (3, 3), activation=None, padding="same")(uconv3) #(none,16,16,128)
...
input_edge2 = Lambda(edge_comp)(input_seg)
...
encoder=Model(inputs = [input_seg,epsilon], outputs =[z,input_edge2],name='encoder')
...
encoder_out,edge_input = encoder([input_seg,epsilon])
...
skin_net = Model(inputs = [input_seg,epsilon],outputs = [decoder_out,seg_out])
...
def cross_corr(y_pred,y_true, eps = 0.001):
    #mx = 0.
    #my = 0.
    #ccloss = 0.
    ...
    sy = K.std(y)
    xnum, ynum = x-mx, y-my
    xden = K.sqrt(K.square(sx) + K.square(eps))
    yden = K.sqrt(K.square(sy) + K.square(eps))
    cx = xnum / xden
    cy = ynum / yden
    ccloss = K.sum(K.square(cx - cy))
    print(ccloss)
    #score = 2. * batch
    #ccloss = ccloss / (2. * batch)
    print(ccloss)
    #batch = predicted.size()[0]
    #batch_loss = 0
    # for index in range(batch):
        
    #     coefficient = self._dice_coefficient(predicted[index], target[index])
    #     batch_loss += coefficient

    #     batch_loss = batch_loss / 2*batch
    return ccloss
...
callbacks = [
    ...
    ModelCheckpoint('./models/vae_u-efficientnet/vae-u-efficientnet_u_unfrez_128_NDBE_latent_search_target_nbi_100_ccloss.h5', verbose=1, save_best_only=True, save_weights_only=True),
    NIR_history
]
...
#load weights
skin_net.load_weights('./models/vae_u-efficientnet/vae-u-efficientnet_u_unfrez_128_NDBE_latent_search_target_nbi_100_ccloss.h5')
...
#show_vae_image(y_test ,test_vae_segment)
...
#write_image(path_images , path_masks ,test_vae_pred ,preds_test_t)
...
# Check if valid data looks all right
plot_sample(X_test, y_test, test_vae_segment, preds_test_t, ix=0)
...
#path_images = './predict_image/miccai_uda/image_ndbe/'
#path_masks = './predict_image/miccai_uda/label_ndbe/'
path_images = './predict_image/miccai_uda/label_pred_ndbe/'
path_masks = './predict_image/miccai_uda/label_test_ndbe/'
# write_image(path_images , path_masks ,test_vae_pred ,y_test  )
write_image(path_images , path_masks ,test_vae_pred ,preds_test_t)
...
def get_gradient(y_true , y_pred , z ):
    ...
    ssim = 1 - cross_corr(y_pred_ssim, y_true_ssim, eps=0.001)
...
#             (test_vae_pred , test_vae_segment)  = skin_net.predict(next_x_images , verbose = 1)
            #next_x_images = tf.image.convert_image_dtype(next_x_images, tf.float32)
            #next_x_images = tf.convert_to_tensor(next_x_images)
            #z , z_edge = encoder([next_x_images,epsilon])
            test_vae_predd = decoder.predict([z,z_edge])
            print(type(z),z.shape,type(test_vae_predd),test_vae_predd.shape , type(next_x_images))
            grad = get_gradient(next_x_images , test_vae_predd , z)
...
#             grad = np.reshape(grad, (64,64))
        ...
        show_vae_image(x_star ,test_vae_predd )
...
ssim = 1 - cross_corr(y_pred, y_true, eps=0.001)
...
ssim1 = (1 - tf.image.ssim((y_pred + 1 )*0.5, (y_true + 1)*0.5, 2.0))
ssim = 1 - cross_corr(y_pred, y_true, eps=0.001)
l_joint = 0.25*ssim + 0.75*ssim1
ssim = l_joint
gradients = K.gradients(ssim, z_tensor)
sess = tf.InteractiveSession()
sess.run(tf.initialize_all_variables())
...
for i in range(gradient_iter):
    ...
    z_grad_ascent = np.random.normal(0,1,size = (64,64))
    z_grad_ascent = np.reshape(z_grad_ascent , (64,64))
    test_vae_predd = test_vae_predd
    show_vae_image(next_x_images , next_x_images)
#     (test_vae_pred , test_vae_segment)  = skin_net.predict(next_x_images , verbose = 1)
    show_vae_image(test_vae_predd ,test_vae_predd )
    for j in range(1):
        for k in range(2):
            print(j,k)
            z_grad_ascent = z_grad_ascent - gamma*V
...
#         x_star = decoder.predict([z_grad_ascent , z_edge] , verbose = 1)
        show_vae_image(test_vae_predd ,x_star )
